...
class AsrWsClient:
    ...
    async def segment_data_processor(self, wav_data: bytes, segment_size: int):
        ...
        header["X-Api-Access-Key"] = "CJeZd0ctydxB5m3kDSBtxByLbKDVQbsz"
        header["X-Api-App-Key"] = "6372825325"
    ...
    async def execute(self, audio_data=None):
        if self.audio_path is not None: # if path is not None, then read audio data from file, override audio_data
...
def execute_one(audio_item, **kwargs):
    ...
    if 'audio_data' in kwargs:
        asr_http_client = AsrWsClient(
            audio_path=audio_path,
            format="pcm",
            **kwargs
        )
        result = asyncio.run(asr_http_client.execute(audio_data=kwargs['audio_data']))
    else:
        asr_http_client = AsrWsClient(
            audio_path=audio_path,
            **kwargs
...
def stream_audio():
    ...
    try:
        while True:
            ...
            if len(audio_data) >= chunk * 100:  # Process every 100 chunks
                result = execute_one(
                    {
                        'id': index,
                        "path": None
                ...
                index += 1
                # print(result)
                audio_data = bytearray()  # Reset buffer after processing
...
def fixed_length_audio(time_length=5):
    audio_format = pyaudio.paInt16
    channels = 1
    rate = 16000
    chunk = 1024
    ...
    audio_data = bytearray()
    try:
        for _ in range(0, int(rate / chunk * time_length)):
            data = stream.read(chunk)
            audio_data.extend(data)
        result = execute_one(
            {
        ...
        if result['result']['is_last_package']:
            asr_text = result['result']['payload_msg']['result']['text']
        # print(result)
    except KeyboardInterrupt:
        print("Recording stopped.")
    finally:
